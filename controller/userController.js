...
export async function register(req, res) {
  ...
  try {
    ...
    const token = jwt.sign(
      { Id: response.rows[0].id, role: response.rows[0].role },
...
export async function getBooks(req, res) {
  ...
  try {
    const result = await db.query(
      "select books.*,COALESCE(ROUND(AVG(reviews.rating),1),0) AS rating  from books left join reviews on books.id=reviews.book_id group by books.id order by created_at DESC limit $1 offset $2",
      [limit, offset]
    );
    const nextBooks = await db.query("select * from books limit 1 offset $1", [
      page * limit,
    ]);
...
export async function getParticularBook(req, res) {
  ...
  try {
    const result = await db.query(
      "select books.*,COALESCE(ROUND(AVG(reviews.rating),1),0) AS rating  from books left join reviews on books.id=reviews.book_id where books.id=$1 group by books.id",
      [id]
    );
    if (result.rows.length === 0) {
...
export async function getReviews(req, res) {
  ...
  try {
    const result = await db.query(
      "select r.rating,r.review,r.created_at,u.username from reviews r join users u on r.user_id=u.id where r.book_id=$1 order by created_at DESC limit $2 offset $3",
      [id, limit, offset]
    );
...
export async function postReview(req, res) {
  ...
  try {
    const result = await db.query(
      "insert into reviews (user_id,book_id,rating,review) values($1,$2,$3,$4) RETURNING *",
...
export async function Search(req, res) {
  ...
  console.log(`Searching for term: ${searchTerm}, Rating filter: ${rating}`);
  try {
    const result = await db.query(
      "select books.*,COALESCE(ROUND(AVG(reviews.rating),1),0) AS rating  from books left join reviews on books.id=reviews.book_id where (LOWER(books.title) like $1 or LOWER(books.author) like $1) group by books.id HAVING (COUNT(reviews.rating) > 0 AND AVG(reviews.rating) >= $2) OR ($2 = 0) order by books.created_at DESC LIMIT $3 OFFSET $4",
      [`%${searchTerm}%`, rating, limit, offset]
    );
    const countResult = await db.query(
      `SELECT COUNT(*) FROM (
                ...
                WHERE (LOWER(books.title) LIKE $1 OR LOWER(books.author) LIKE $1)
                GROUP BY books.id
                HAVING (COUNT(reviews.rating) > 0 AND AVG(reviews.rating) >= $2) OR ($2 = 0)
            ) AS filtered`,
      [`%${searchTerm}%`, rating]
    );
    const total = parseInt(countResult.rows[0].count);
    const hasMore = offset + limit < total;
    res.status(200).json({ books: result.rows, hasMore });
  } catch (err) {
    console.error(err);
    res.status(500).json({ error: err.message });
  }
...
export async function AddBook(req, res) {
  ...
  try {
    const result = await db.query("select * from books where LOWER(title)=$1", [
    ...
    const response = await db.query(
      "insert into books (title,description,author,published_year) values($1,$2,$3,$4) RETURNING *",
      [title, description, author, Published_year]
    );
    if (response.rows.length === 0) {
